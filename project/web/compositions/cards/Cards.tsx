...
export default function Cards({}) {
    ...
    if (selectedCategoryInfo === undefined) {
      ...
      const filtrado = news.filter(
        (noticia) =>
          ...
          selectedCategoryInfo.categoria.toLowerCase()
      );
  ...
  return (
    <DestaquesStyled>
      <OpcoesNews
        onClick={(categoria: EnumCategoria) => comparaCategoria({ categoria })}
        opcoes={opcoes}
      />
      <CardsStyled>
        {(filteredNews || news) &&
          (filteredNews || news).map((noticia) => (
            <Card
              ...
              minutagemOuPublicacao={"Publicado:"}
            />
          ))}
        {!filteredNews && !news && <Text tag="h1">Nada a vista</Text>}
      </CardsStyled>
    </DestaquesStyled>
  );
}
